...
namespace PasCompiler {
    class Token {
        ...
        public bool IsAdditiveOperator => _additiveOperators.Contains(Kind);

        public bool IsMultiplicativeOperator => _multiplicativeOperators.Contains(Kind);

        public bool IsConiditionalOperator => _conditionalOperators.Contains(Kind);

        public override string ToString() => $" ({Kind}, {Value})";

        static E[] _additiveOperators = { Plus, Minus };   // Arithmetic Operators

        static E[] _multiplicativeOperators = { Mul, Div };  // Multiplicative Operators

        static E[] _conditionalOperators = { Greater, Less, Geq, Leq, Neq, And, Or, Not };   // Conditional Operators
    }
    ...
    class Lexer {
        ...
        Token GetToken() {
            ...
            return ch switch {
                '{' => new Token(OpenBrace),
                '}' => new Token(ClosedBrace),
                '[' => new Token(OpenBracket),
                ']' => new Token(ClosedBracket),
                '(' => new Token(OpenParen),
                ')' => new Token(ClosedParen),
                '.' => new Token(Period),
                ',' => new Token(Comma),
                '=' => new Token(Eq),
                ';' => new Token(SemiColon),
